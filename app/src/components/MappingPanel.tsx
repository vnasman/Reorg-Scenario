...
export function MappingPanel({
  ...
  function setField(field: TargetField, col: string | null) {
    setMapping((m) => {
      ...
      // En källkolumn kan bara mappas till ett fält — släpp den från andra
      if (col) {
  ...
  return (
    <div className="mt-12">
      <div className="flex items-baseline justify-between">
        <div>
          <div className="text-xs uppercase tracking-wider text-ink-500">Kolumnmappning</div>
          <div className="mt-1 text-sm text-ink-700">
            <span className="font-medium text-ink-900">{sheet.fileName}</span> • {sheet.rows.length} rader • {sheet.headers.length} kolumner
          </div>
        </div>
      ...
      {matchedProfile && activeProfileId === matchedProfile.id && (
        <div className="mt-3 text-xs text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-md px-3 py-2">
          Kände igen filformatet — profilen "{matchedProfile.name}" har applicerats automatiskt.
        </div>
      )}

      {profiles.length > 0 && (
        <div className="mt-4 flex items-center gap-2 flex-wrap">
          <span className="text-xs text-ink-500">Sparade profiler:</span>
          {profiles.map((p) => (
            <span key={p.id} className="inline-flex items-center gap-1">
              ...
              <button onClick={() => removeProfile(p.id)} className="text-ink-300 hover:text-red-600 text-xs" title="Ta bort profil">
                ×
              </button>
            </span>
          ))}
        ...
        <table className="w-full text-sm">
          <thead className="bg-ink-100 border-b border-ink-300">
            <tr className="text-left text-[11px] uppercase tracking-wider text-ink-500">
              ...
              <th className="px-3 py-2">Kolumn i filen</th>
              <th className="px-3 py-2">Exempelvärde</th>
            </tr>
          </thead>
          <tbody>
            {FIELD_META.map((f) => (
              <tr key={f.field} className="border-b border-ink-100 last:border-0">
                ...
                <td className="px-3 py-2">
                  ...
                  >
                    <option value="">— Mappa inte</option>
                    {sheet.headers.map((h) => (
      ...
      {missingRequired.length > 0 && (
        <div className="mt-3 text-xs text-red-700">
          Obligatoriska fält saknas: {missingRequired.map((f) => f.label).join(', ')}
        </div>
      )}
      {duplicateIds > 0 && (
        <div className="mt-2 text-xs text-amber-700">
          Varning: {duplicateIds} dubblerade anställnings-ID i filen — senare rader skriver inte över tidigare, men hierarkin kan bli fel.
        </div>
      )}

      {preview.length > 0 && (
        <div className="mt-5">
          <div className="text-xs uppercase tracking-wider text-ink-500 mb-2">Förhandsgranskning</div>
            ...
            <table className="w-full text-xs">
              <thead className="bg-ink-100 border-b border-ink-300">
                <tr className="text-left text-[10px] uppercase tracking-wider text-ink-500">
                  ...
                  <th className="px-2 py-1.5">Chef</th>
                  <th className="px-2 py-1.5">Avdelning</th>
                  <th className="px-2 py-1.5">Ort</th>
                  <th className="px-2 py-1.5">Lön</th>
                  <th className="px-2 py-1.5">FTE</th>
                  <th className="px-2 py-1.5">Kön</th>
                  <th className="px-2 py-1.5">Födelseår</th>
                  <th className="px-2 py-1.5">Nivå</th>
                </tr>
              </thead>
              <tbody>
                {preview.map((e) => (
                  <tr key={e.employee_id} className="border-b border-ink-100 last:border-0">
                    ...
                    <td className="px-2 py-1.5">{e.salary ? e.salary.toLocaleString('sv-SE') : '—'}</td>
          ...
          {!mapping.level && <div className="mt-1.5 text-[11px] text-ink-500">Nivå härleds från chefshierarkin (toppnivå = 1).</div>}
        </div>
      )}
